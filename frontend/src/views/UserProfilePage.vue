  ...
  <div class="user-profile-page">
    ...
    <div class="profile-container">
      <div class="avatar-section">
        ...
        <h1>{{ userInfo?.nickname || '加载中...' }}</h1>
      </div>

      <div class="bio-section">
        <div v-if="userInfo.email" class="tag">{{ userInfo.email }}</div>
...
onMounted(async () => {
  try {
    ...
    if (res.code === 200) {
      ...
      // 处理用户统计数据
      if (res.data) {
        userInfo.value.followerCount = res.data.followerCount
        userInfo.value.followingCount = res.data.followingCount
        userInfo.value.articleCount = res.data.articleCount
      }
    } else {
